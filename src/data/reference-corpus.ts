...
//
// **문서 자체의 섹션 순서는 여기가 아니다.** 그것은 축 (1)
// `structureArchetypes[].sectionFlow`가 담는다. `flow`는 "스킬이 시키는 일"의
// 순서이고, `sectionFlow`는 "SKILL.md를 어떤 섹션 순서로 쓸까"다. 섞지 말 것.
//
...
export const referenceCategories: ReferenceCategory[] = [
	{
		...
		patterns: [
			...
			},
			{
				...
				detail:
					"변명 표와 같은 항목이 여기 다시 나와도 된다. 하는 일이 다르기 때문이다 — 변명 표는 설득하고, 이 목록은 알아채게 한다. 그래서 여기서는 사유를 달지 않고 신호만 짧게 적고, 목록 끝에 '이 중 하나라도 해당하면 무엇을 하라'는 처방을 한 줄로 붙인다.",
				role: "constraint",
				kind: "artifact",
				format: {
	...
	},
	{
		...
		patterns: [
			...
			},
			{
				...
				detail:
					"만든 스킬을 검증하지 않고 배포하려 할 때 나오는 여덟 가지다. 반박은 원칙이 아니라 인과로 되받는다. 표 끝은 한 문장으로 닫아 개별 행이 협상거리가 되지 않게 한다 — '전부 같은 뜻이다: 배포 전에 테스트하라. 예외 없다'. 변명 목록은 상상해서 채우지 않고 실제 테스트에서 나온 것을 모아 만든다.",
				role: "output-rule",
				kind: "artifact",
				format: { count: "8행" },
				examples: [
	...
	},
	{
		...
		patterns: [
			...
			},
			{
				...
				detail:
					"섹션마다 5~20개 옵션을 브레인스토밍하고 선별해 초안을 쓴 뒤 피드백을 반영한다. 고칠 때는 문서 전체를 다시 출력하지 말고 바뀐 부분만 편집한다.",
				role: "workflow-step",
				// 정책 B 등급 2 — 라이선스 미확인 소스이므로 개념만 우리 말로 요약했다.
				adapted: true,
				sourceIds: ["anthropic-doc-coauthoring"],
