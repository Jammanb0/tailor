		...
		patterns: [
			{
				...
				detail:
					"글쓴이는 알고 읽는 쪽은 모르는 것이 문서를 망친다. 그래서 초안 전에 맥락부터 채운다. 받을 때는 정리해서 달라고 하지 말고 있는 대로 쏟아내게 한다 — 정리를 요구하면 정리하기 어려운 것이 빠지는데, 그게 대개 가장 중요한 것이다. 짧게 답해도 되고 형식이 없어도 된다고 먼저 알린다. **언제까지 모아야 하는지가 이 절차의 핵심이다** — 기초를 다시 설명받지 않고도 예외 상황과 맞바꿈을 물을 수 있게 되면 충분히 모은 것이다. 이 조건이 없으면 '충분히 모았다'가 판단할 수 없는 말이 된다. **그래서 받는 동안 계속 추적한다** — 지금까지 무엇을 알게 됐고 무엇이 아직 불분명한지. 추적하지 않으면 종료 조건을 판정할 재료가 없다.",
				role: "workflow-step",
				kind: "artifact",
				format: {
				...
				adapted: true,
				// DC-04(틀이나 기존 문서를 먼저 읽는다)는 위 exception이,
				// DC-07(정리하지 말고 쏟아내게 한다)은 detail이 이미 담고 있어
				// 번호만 보탠다. DC-10(무엇이 불분명한지 추적)은 detail에 넣었다.
				auditIds: [
					"DC-02",
					"DC-03",
					"DC-04",
			...
			},
			{
				...
				// 담고(2026-08-21 정정), 원문의 질문 문장·예시 문구는 담지 않았다.
				adapted: true,
				// DC-19(브레인스토밍할 때 볼 것 둘)는 위 flow의 brainstorm 단계 gate가,
				// DC-22(빠진 게 없는지 한 번 더)는 gap 단계가 이미 담고 있어 번호만 보탠다.
				auditIds: [
					"DC-13",
					"DC-14",
					"DC-15",
			...
			},
			{
				...
				detail:
					"쓴 사람은 이미 알기 때문에 무엇이 빠졌는지 못 본다. 그래서 이 대화의 맥락을 하나도 모르는 쪽에게 문서만 주고 읽힌다. 먼저 독자가 실제로 물을 법한 질문을 예측해 그것부터 물어보고, 무엇을 맞히고 무엇을 틀렸는지 정리한다. 틀린 곳이 나오면 그 섹션의 다듬기로 되돌아간다. **언제 끝나는지가 정해져 있다** — 일관되게 맞히고 새로운 빈틈이 더 안 나오면 끝이다. 읽히는 일을 대신 맡길 수단이 있으면 직접 돌리고, 없으면 **사용자가 손으로 할 수 있게 절차를 적어준다** — 수단이 없다는 이유로 이 단계를 건너뛰지 않는다.",
				role: "verification",
				kind: "artifact",
				format: {
				...
				// 담고(2026-08-21 정정), 원문의 질문 문장·예시 문구는 담지 않았다.
				adapted: true,
				// DC-34(맡길 수단이 있으면 직접, 없으면 사용자가 하도록 안내)를
				// detail에 넣었다. 원문은 특정 런타임의 하위 에이전트를 전제하지만,
				// 옮긴 것은 "수단 유무로 절차를 가르되 건너뛰지는 않는다"는 알맹이다.
				auditIds: [
					"DC-28",
					"DC-29",
					"DC-30",
			...
			{
				id: "images-need-words",
				summary: "그림에 기대는 내용은 글로도 읽히게 남긴다",
				detail:
					"문서에 그림을 넣으면 그 그림에만 있는 정보가 생긴다. 나중에 이 문서를 읽는 쪽이 그림을 못 보는 경우가 흔하다 — 화면 낭독기를 쓰거나, 텍스트만 복사해 가거나, AI에게 읽히는 경우다. 그때 그림에만 있던 내용은 통째로 사라진다. 그림마다 무엇을 보여주는지 글로 적어두고, 고칠 문서에 설명 없는 그림이 있으면 짚어서 알린다.",
				role: "output-rule",
				kind: "artifact",
				// 정책 B 등급 2 — 개념만 우리 말로 요약했다. 원문은 특정 도구의
				// 대체 텍스트 기능을 전제하지만, 옮긴 것은 "그림에만 있는 정보를
				// 남기지 마라"는 알맹이다.
				adapted: true,
				auditIds: ["DC-05"],
				sourceIds: ["anthropic-doc-coauthoring"],
			...
			{
				id: "document-is-not-finished",
				summary:
					"다 썼다고 끝이 아니다 — 깊이는 부록으로, 갱신은 실제 반응으로",
				detail:
					"본문에 다 넣으려 하면 문서가 비대해지고, 빼면 근거가 사라진다. 부록이 그 자리다 — 본문은 읽히는 분량으로 두고 깊이는 부록으로 내린다. 만든 과정 자체도 부록으로 링크해두면 읽는 쪽이 '왜 이렇게 됐는지'를 되짚을 수 있다. 그리고 문서는 낸 시점이 아니라 **실제 독자의 반응이 돌아온 뒤에** 한 번 더 손봐야 제 모양이 된다.",
				role: "output-rule",
				kind: "artifact",
				// 정책 B 등급 2 — 개념만 우리 말로 요약했다.
				adapted: true,
				auditIds: ["DC-37"],
				sourceIds: ["anthropic-doc-coauthoring"],
			},
